...
export default async function handler(req, res) {
  try {
    if (req.method === 'GET') {
      ...
      // one-time load of the starter list; the flag stops it ever running twice.
      // Appends, so anything already in the list is kept.
      if (!(await hasSeeded())) {
        const existing = new Set(items.map((i) => i.title.trim().toLowerCase()));
        const fresh = SEED_TITLES.map((t) => clean(t, 300))
